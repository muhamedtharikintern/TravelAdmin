...
const VehicleInsuranceUpload = ({ navigation, route }) => {
  ...
  const uploadToFirebase = async (uri) => {
    const reference = storage().ref(`insurance/insurance_${mobileNo}_${Date.now()}.jpg`);
    await reference.putFile(uri);
    return await reference.getDownloadURL();
  };

  const handleSubmit = async () => {
    if (!image) return Alert.alert('Error', 'Please upload your insurance document');
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/auth/upload-insurance`, {
        method: 'POST',
        headers: {
