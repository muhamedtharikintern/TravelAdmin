...
const FitnessCertificate = ({ navigation, route }) => {
  ...
  const uploadToFirebase = async (uri) => {
    const reference = storage().ref(`fitness/fitness_${mobileNo}_${Date.now()}.jpg`);
    await reference.putFile(uri);
    return await reference.getDownloadURL();
  };

  const handleSubmit = async () => {
    if (!image) return Alert.alert('Error', 'Please upload your fitness certificate');

    try {
      setLoading(true);
      setUploading(true);
      const fitnessUrl = await uploadToFirebase(image);
      setUploading(false);

      const response = await fetch(`${API_URL}/auth/upload-fitness`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        },
        body: JSON.stringify({ fitnessUrl }),
      });

      const result = await response.json();

      if (result.success) {
        Alert.alert('Success', 'Fitness certificate uploaded successfully!');
        navigation.navigate('DutyDashboard', { mobileNo, token }); // ✅ pass params
      } else {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Fitness Certificate</Text>
      </View>

      {/* CONTENT */}
      <View style={styles.content}>
        <Text style={styles.label}>Upload Fitness</Text>
